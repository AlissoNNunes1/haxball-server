...
room.onPlayerJoin = function (player) {
  ...
  whisper('Olá, seja bem vindo a CHA! Um servidor de Real Soccer', player.id, 0x61ddff, 'bold', 0);
...
room.onPlayerChat = function (player, message) {
  ...
  if ((choosePositionMode && !message.startsWith('')) || (sala_mutada && !player.admin))
    return false;
  ...
  if (message.startsWith('!')) {
    ...
    if (
      Object.keys(positions[activeFormation]).includes(args[0].toUpperCase()) &&
      player.team !== 0
    ) {
...
function anuncio() {
  room.sendAnnouncement('Discord CHA: https://discord.gg/mWzatsxjTA', null, azul, 'bold', 0);
}
