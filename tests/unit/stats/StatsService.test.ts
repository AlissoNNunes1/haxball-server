...
import { AdvancedMatchStats, BasicMatchStats } from '../../../src/stats/types';
...
describe('StatsService', () => {
  ...
  it('saveBasicStats inserts row into stats table', async () => {
    ...
    const row = sqlite
      .prepare('SELECT * FROM stats WHERE account_id = ? AND match_id = ?')
      .get(accountId, 1234);
  ...
  it('updatePlayerAggregate creates aggregate record', async () => {
    ...
    // Verifica wins/losses calculados corretamente
    expect(aggRow.total_wins).toBe(1);
    expect(aggRow.total_losses).toBe(1);
    expect(aggRow.win_rate).toBeGreaterThanOrEqual(0);
  });
});
